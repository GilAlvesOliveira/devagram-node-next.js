...
export const validarTokenJWT = (handler : NextApiHandler) =>
 (req : NextApiRequest, res : NextApiResponse) => {
    ...
    try{
        ...
        if(!MINHA_CHAVE_JWT){
            return res.status(500).json({ erro : 'ENV chave JWT nao informada na execucao do prejeto'});
        }
        ...
        if(req.method !== 'OPTIONS'){
            ...
            if(!decoded){
                return res.status(401).json({erro: 'Nao foi possivel validar o tokem de acesso'})
            }
